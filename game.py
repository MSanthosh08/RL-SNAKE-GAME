...
class SnakeGameAI:
    def __init__(self, w=640, h=480, render=True):
        ...
        if self.render:
            ...
            pygame.display.set_caption("RL Snake v4")
        else:
            # offscreen surface so we can still draw if needed
    ...
    def play_step(self, action):
        """action: one-hot np.array of length 3 (0: straight, 1: right, 2: left)"""
        ...
        if self.head == self.food:
            ...
            reward = 30.0
            self._place_food()
            self.frame_iteration = 0  # reset stagnation counter
        else:
            self.snake.pop()
            new_distance = self._distance_to_food()
            # small shaping toward food
